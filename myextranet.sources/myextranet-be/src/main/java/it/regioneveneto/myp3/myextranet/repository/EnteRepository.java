...
public interface EnteRepository extends JpaRepository<Ente, Integer>, JpaSpecificationExecutor<Ente> {
	...
	@Query("select e from Ente e where e.codIpa = :codIpa AND now() >= e.dtInizioVal AND now() < e.dtFineVal")
	List<Ente> getEntiByCodIpaAndValidity( @Param("codIpa") String codIpa);

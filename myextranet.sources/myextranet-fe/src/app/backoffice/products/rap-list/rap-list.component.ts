...
@Component({
  selector: 'app-rap-list',
  templateUrl: './rap-list.component.html',
  styleUrls: ['./rap-list.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class RAPListComponent implements OnInit, AfterViewInit {

  @ViewChild('morfeoRAPconfermati') morfeoFilter: EngDynamicFormsComponent;
  ...
  public formJson: IForm = {
    ...
    components: [
      ...
      {
        key: 'flgAttivo', type: 'select', label: 'RAP attivi', data: {
          "values": [
            {
              "label": "Si",
              "value": 1
  ...
  getDataFromService(pageindex: number = 1, pagesize: number = 10, sortBy?: string, filter?: any, order?: string): void {
    this.isLoading = true;
    let url = `/rappresentante-ente-rap?_page=${pageindex}&_pageSize=${pagesize}&flgConferma=1`;
  ...
  public navigateToDetail(row?: { idRappr: number }): void {
    ...
    this.router.navigate(['/backoffice/RAP/detail'], { queryParams: { idRappr: row.idRappr } });
